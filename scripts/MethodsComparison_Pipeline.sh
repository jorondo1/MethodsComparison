...
export RA_TSV=$MC/Feces_RA/preproc/preprocessed_reads.sample.tsv
...
$bracken --confidence 0.20 --sample_tsv $RA_TSV --out $MC/Feces_RA/KB20
...
$metaphlan --sample_tsv $RA_TSV --db $FAST/metaphlan4_db/mpa_vOct22_CHOCOPhlAnSGB_202212 --out $MC/Feces_RA/MPA_db2022
...
$metaphlan --sample_tsv $RA_TSV --db $FAST/metaphlan4_db/mpa_vJun23_CHOCOPhlAnSGB_202307 --out $MC/Feces_RA/MPA_db2023
...
:> $MC/Feces_RA/raw/samples_to_process.tsv
while read p; do 
SAM=$(echo $p | cut -f1 -d' ')
if [[ "$SAM" == "run_accession" ]]; then 
continue
fi
f1=$(find $MC/Feces_RA/raw -name "${SAM}_1.fastq.gz")
f2=$(find $MC/Feces_RA/raw -name "${SAM}_2.fastq.gz")
echo -e "$SAM\t${f1}\t${f2}" >> $MC/Feces_RA/raw/samples_to_process.tsv
done < $MC/Feces_RA/raw/metadata.tsv 

#Process fecal samples
mkdir -p $MC/Feces_RA/preproc
bash $ILL_PIPELINES/generateslurm_preprocess.kneaddata.sh \
	--sample_tsv $MC/Feces_RA/raw/samples_to_process.tsv \
	--out $MC/Feces_RA/preproc \
